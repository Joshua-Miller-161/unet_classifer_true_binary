...
def variance_scaling(scale, mode, distribution,
  ...
  def init(shape, dtype=dtype, device=device):
    #print(" >> >> INSIDE variance_scaling  init shape", shape)
...
class Upsample(nn.Module):
    def __init__(self,
                 ...
                 with_conv: bool = True,
                 fir: bool = False,
                 fir_kernel: Optional[list] = None
        ):
        super().__init__()
        self.with_conv = with_conv
        self.conv = ddpm_conv3x3(in_ch, in_ch) if with_conv else nn.Identity()
...
class Downsample(nn.Module):
    def __init__(self,
        ...
        if with_conv:
            ...
            self.conv.weight.data = default_init()(self.conv.weight.shape)
        ...
        self.register_buffer(
            "fir_kernel",
            torch.tensor(fir_kernel, dtype=torch.float32) if fir_kernel is not None else None
        )
...
def get_timestep_embedding(timesteps, embedding_dim, max_positions=10000):
  assert len(timesteps.shape) == 1, "Got:"+ str(len(timesteps.shape))+", "+str(timesteps.shape)
